...
encrypted_password.encode('utf-8')):
       ...
       return False
def main():
   parser = argparse.ArgumentParser(description='Bcrypt cracker')
   parser.add_argument('-p', '--passwordfile', help='Path to the password
file')
   parser.add_argument('-d', '--dictionary', help='Path to the password
dictionary file')
   ...
   if args.passwordfile:
       with open(args.passwordfile, 'r') as file:
           ...
           if len(lines) >= 1:
               ...
               if args.dictionary:
                   with open(args.dictionary, 'r', encoding='latin-1') as
dict_file:
                       password_dictionary = dict_file.read().splitlines()
               else:
                                  password_dictionary = ["password", "123456", "qwerty",
"admin"]
               for password_guess in tqdm(password_dictionary, desc="Cracking
bcrypt"):
                   if crack_password(password_guess, password, salt):
                       break
           else:
               print("Invalid password file. It should have at least 1 line
(password).")
   else:
